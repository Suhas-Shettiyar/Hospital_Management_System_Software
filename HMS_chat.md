...
### 2026-07-20 (latest) — Permission-based RBAC, role rename, and two new department packages: Queue (front-desk tokens) and OPD (consultations)
Picks up the OPD/appointments work that was left uncommitted at the end of the 2026-07-18 design pass. Session covered three connected pieces: a real permission system (replacing role-name checks), a role rename to match it, and the first two real department packages built on the Stage 3/4 plugin+federation framework (Queue as a core module in `web-shell`, OPD as a genuinely separate federated remote on port 5175).

**RBAC overhaul (`backend/app/core/auth/permissions.py`, `dependencies.py`):** `require_roles(*roles)` replaced with `require(permission)` — a plain `ROLE_PERMISSIONS: dict[str, set[str]]` map (kept free of FastAPI imports so it's independently unit-testable) resolves a role to its permission strings (`patients:read`, `queue:write`, `consultation:write`, etc.). `UserOut` gained a computed `permissions` field so `/me` returns the resolved list — the frontend never duplicates the map, it just checks `user.permissions`. New `GET /api/auth/staff?role=doctor` (gated on `queue:read`) backs the queue board's doctor picker, returning only `ACTIVE` accounts via a lightweight `StaffOut` shape (no email/permissions).

**Role rename, via migration not just a model edit:** `pharmacy` → `pharmacist`, `billing` → `cashier`, plus a new `frontdesk` role — done as a real Alembic migration (`20260720_4b1f7c9a2d63`) that widens the `users.role` column before an `UPDATE ... SET role = ...` data migration, not just a code-side rename, since existing rows needed their stored values changed.

**Frontend permission plumbing:** `AuthProvider` derives a memoized `permissionSet` (`Set<string>`) from `user.permissions`; new `useCan(permission)` hook and `<Can>` component do O(1) cosmetic checks. `Sidebar` now filters nav items by a module's optional `permissions` array (modules that don't declare any stay visible to everyone, so nothing existing broke). New `RequirePermission` route wrapper in `AppShell.tsx` shows a 403 `Result` page for a disallowed route. **Explicitly documented in the code itself, twice:** all of this frontend gating is cosmetic UX only — the real enforcement is always the backend's `require(permission)` dependency; a user who bypasses the UI still gets a real 403 from the API.

**New: Appointments/Queue module** (`backend/app/modules/appointments/`, `frontend/web-shell/src/features/queue/`) — built as a **core** web-shell feature, not a federated remote (small enough not to need the isolation). Front-desk token board: check in a patient against a doctor → token gets a per-doctor-per-day sequence number → progresses `waiting → in_consult → done` via a 3-column live board (`refetchInterval: 5000`). Deliberately decoupled from `OpdVisit` — calling a token forward doesn't create a visit, and starting a visit doesn't require a token; related workflows, not one mechanism. Gated on `queue:read`/`queue:write`.

**New: OPD (Outpatient/Consultation) module** — the first real federated remote beyond the `example_hello` proof-of-concept, and the first to duplicate the `HmsModule`/`HmsMenuItem` contract shape locally (`frontend/modules/opd/src/moduleContract.ts`) rather than import it, since separate-npm-project isolation is the whole point of federation. Runs on port 5175 (`opd_remote`), wired into `web-shell/vite.config.ts` remotes + `federation.d.ts` + `loadRemoteModules.ts` the same way Stage 4 established. Backend side: new `opd_visits`/`opd_diagnoses`/`opd_prescriptions`/`opd_prescription_items` tables (`backend/app/core/opd/models.py`, migration `20260716_2587a49be27b` — foreign keys point at the shared core `patients`/`users` tables, per the Technical Roadmap's "core patients shared across department schemas" design) and `backend/app/modules/opd/router.py` (patient search/register scoped to OPD's own consult-focused UI, visit CRUD, add-diagnosis, add-prescription with line items, complete-visit — each gated on the appropriate permission: `patients:read/write`, `queue:write` for opening/updating a visit, `consultation:write` for diagnosis/prescription/complete). ICD-10 codes and medicine names are plain free-text for now (no coded lookup table exists yet) — documented in the model docstrings as a deliberate simplification, not an oversight.

**Both new packages registered the same way Stage 3 established:** added to `KNOWN_PACKAGE_MODULES` in `plugins/loader.py`, each with its own `plugin.py` exposing `module_manifest()`/`module_router()` via the pluggy hook contract, router.py staying the single source of truth for both.

**Verified before committing:** backend imports cleanly end-to-end (`python -c "import app.main"`, all new routers/models load with the renamed roles and new permission map). Both frontend projects (`web-shell` and the new `modules/opd` remote) type-check clean (`tsc --noEmit`, zero errors). No leftover TODO/FIXME markers in any new file.

**Housekeeping note, not yet acted on:** `frontend/modules/opd/package-lock.json` was generated against this machine's corporate npm registry mirror (`elilillyco.jfrog.io`), same contamination class fixed once before in `web-shell`'s lockfile back on 2026-07-16 (that fix was never fully clean either — one straggler line crept back into `web-shell`'s own lockfile since then). Left as-is for this commit since it doesn't block anything locally; worth a regenerate-from-public-registry pass before this ever needs to install cleanly on a machine without that mirror configured.

**Not committed yet at start of session** — this whole feature set (RBAC, role rename, Queue, OPD) had been sitting uncommitted in the working tree since 2026-07-18, deliberately left out of that session's frontend-redesign commit. Staged and pushed to `origin/main` as part of wrapping up this session.

**What's next:** the OPD/Queue pair covers FR-OPD/FR-APPT's MVP shape; still open per the roadmap: real ICD-10 lookup, a pharmacy medicine master (referenced but not built — prescriptions are still free-text), lab and pharmacy department packages themselves (roles already exist: `lab`, `pharmacist`, `cashier`), and an admin-only staff-creation endpoint (self-registration for non-admin roles is still a known, documented gap).
